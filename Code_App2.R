...
Application <- read.csv("C:/Users/gergo/Desktop/Github/MasterThesisFinal/Application2.csv", sep=";")
...
top_freq_bigram<-frequent_bigrams[frequent_bigrams$n>60,]#####################CHANGE 1% of Reviews
...
top_freq_trigram<-frequent_trigrams[frequent_trigrams$n>22,]#####################CHANGE 1% of Reviews
...
ll <- ggplot(res, aes(n_topics)) +                    
  ...
  geom_vline(xintercept = 17, color = "black", linetype = "dashed", size = 1) +
...
for (i in 1:ceiling(n_topics/perplot)) {
  p <- text_top_terms %>%
    ...
    theme_classic(base_family = "Century") +
    labs(x = NULL, y = NULL) +
    ggtitle("Identified LDA Topics of Application2") +
...
new_order <- c("14", "10", "12", "13", "3", "5", "11", "7", "8", "1", "2", "9", "4", "6")
...
ggplot(TopicsAll, aes(fill = Most_Likely_Topic, y = relative_count, x = factor(Time))) +
  ...
  labs(x = "Time", y = "Relative Frequency", fill = "Most Likely Topic") +
  ggtitle("Application 2") +
  theme_minimal()+
  theme_classic(base_family = "Century") 
...
TopicsSentiment$Sentiment <- ifelse(TopicsSentiment$Most_Likely_Topic %in% c(14,10,12), 
                                    "Negative", 
                                    ifelse(TopicsSentiment$Most_Likely_Topic %in% c(13,3,5,11,7,8,1,2,9), 
                                           "Neutral", 
                                           "Positive"))
...
ggplot(Application_Emotion_long, aes(fill = emotion, y = relative_value, x = factor(Time))) +
  ...
  geom_text(aes(label = sprintf("%.1f%%", relative_value * 100)), 
            position = position_fill(vjust = 0.5), size = 2, color = "black") +
  labs(x = "Time", y = "Relative Frequency", fill = "Emotion") +
  ggtitle("Relative Stacked Bar Plot of Emotions") +
  theme_minimal()+
  theme_classic(base_family = "Century") 
############
...
modeldata<- Application[,c(51,50,6,8:35)] #####################CHANGE 1%
modeldata <- modeldata %>%
  mutate_if(is.logical, as.factor)
modeldata$Most_Likely_Topic<- as.factor(modeldata$Most_Likely_Topic)
modeldata[,c(3,24:31)]<-scale(modeldata[,c(3,24:31)])
...
set.seed(1)
lasso_model <- cv.glmnet(X_train, y_train, alpha = 1, family = "binomial", nfolds = 20)  
plot(lasso_model)
best_lambda <- lasso_model$lambda.min
set.seed(1)
lasso_model_best <- glmnet(X_train, y_train, alpha = 1, lambda = best_lambda, family = "binomial")
predictions_train <- predict(lasso_model_best, newx = X_train, type = "response")  
...
for (i in 1:nrow(significant_coefficients)) {
  if(significant_coefficients$Estimate[i] < 0){
    significant_coefficients$odds_return[i] <- (1-(exp(significant_coefficients$Estimate[i]))*100)} else {
      significant_coefficients$odds_return[i] <- (exp(significant_coefficients$Estimate[i])-1)*100}}
...
avgSentScore<- Application %>%
  group_by(Time)%>%
  summarise( mean = mean(Sentiment))

sentiment_line_plot <- ggplot(avgSentScore, aes(x = Time, y = mean, color = "Avg .Sentiment Score")) +
  geom_line() +
  geom_point() +
  ...
  theme_minimal() +
  scale_x_continuous(breaks = unique(avgSentScore$Time))+
  theme_classic(base_family = "Century") +
  ylim(0, 0.5)
sentiment_line_plot
...
TopicsPositive<-TopicsAll[TopicsAll$Most_Likely_Topic %in% c(3,4,5),]
...
TopicsNegative<-TopicsAll[TopicsAll$Most_Likely_Topic %in% c(1,6,7),]
color_palette4 <- brewer.pal(n = 4, name = "Set2")
ggplot(TopicsNegative, aes(fill = Most_Likely_Topic, y = relative_count, x = factor(Time))) +
  scale_fill_manual(values = color_palette4) +
  geom_bar(position = "stack", stat = "identity") +
  geom_text(aes(label = sprintf("%.1f%%", relative_count * 100)), 
            position = position_stack(vjust = 0.5), size = 2, color = "black") +
  labs(x = NULL, y = NULL, fill = "Negative Topics:") +
  ggtitle("Portion of Significant Negative Topics") +
  theme_minimal()+
  theme_classic(base_family = "Century")+
  ylim(0,1)
...
Sentences <- Application[,c(1,3,4,5,50)] %>%
...
predictions_train <- predict(logit_model, newx = X_train, type = "response")  
...
modeldataRF<- Application[,c(51,50,6,8:35)]
...
rf_model <- randomForest(Target ~., data = train_dataRF, mtry = 4)
plot(rf_model)
...
ggplot(pdp_results_sentiment, aes(x = Sentiment, y = yhat)) +
  geom_line(color = color_palette[1], size = 2) +  
...
pdp_results_v4 <- partial(rf_model,pred.var = "V5", prob = TRUE, which.class = 2L, type = "classification")
