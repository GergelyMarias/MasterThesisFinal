...
Application <- read.csv("C:/Users/gergo/Desktop/Github/MasterThesisFinal/Application3.csv", sep=";")
...
top_freq_bigram<-frequent_bigrams[frequent_bigrams$n>50,]#####################CHANGE 1% of Reviews
...
top_freq_trigram<-frequent_trigrams[frequent_trigrams$n>17,]#####################CHANGE 1% of Reviews
...
new_order <- c("7", "4", "11", "10", "3", "16", "8", "17", "1", "2", "14", "5", "13", "15","9","6","12")
...
ggplot(TopicsAll, aes(fill = Most_Likely_Topic, y = relative_count, x = factor(Time))) +
  ...
  ggtitle("Application 3") +
  theme_minimal()+
  theme_classic(base_family = "Century") 

TopicsSentiment<- TopicsAll
TopicsSentiment$Sentiment<- NA
TopicsSentiment$Sentiment <- ifelse(TopicsSentiment$Most_Likely_Topic %in% c(7,4,11,10,3,16), 
                                    "Negative", 
                                    ifelse(TopicsSentiment$Most_Likely_Topic %in% c(8,17,1,2,14,5,13,15,9), 
                                           "Neutral", 
                                           "Positive"))
...
modeldata<- Application[,c(56,55,6,8:37)] #####################CHANGE 1%
modeldata <- modeldata %>%
  mutate_if(is.logical, as.factor)
modeldata$Most_Likely_Topic<- as.factor(modeldata$Most_Likely_Topic)
modeldata[,c(3,26:33)]<-scale(modeldata[,c(3,26:33)])
...
TopicsPositive<-TopicsAll[TopicsAll$Most_Likely_Topic %in% c(3,5,7,9),]
...
TopicsNegative<-TopicsAll[TopicsAll$Most_Likely_Topic %in% c(6),]
color_palette4 <- brewer.pal(n = 4, name = "Set2")
...
Sentences <- Application[,c(1,3,4,5,55)] %>%
...
modeldataRF<- Application[,c(56,55,6,8:37)]
